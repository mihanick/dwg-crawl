...
namespace RTree
{
    ...
    public class RTree<T>
    {
        ...
        private void init()
        {
            ...
            if (maxNodeEntries < 2)
            {
                cDebug.WriteLine("Invalid MaxNodeEntries = " + maxNodeEntries + " Resetting to default value of " + DEFAULT_MAX_NODE_ENTRIES);
            ...
            {
                cDebug.WriteLine("MinNodeEntries must be between 1 and MaxNodeEntries / 2");
            ...
            cDebug.WriteLine("init() " + " MaxNodeEntries = " + maxNodeEntries + ", MinNodeEntries = " + minNodeEntries);
        }
        ...
        {
            cDebug.WriteLine("Adding rectangle " + r + ", id " + id);
        ...
        private bool delete(Rectangle r, int id)
        {
            ...
            while (foundIndex == -1 && parents.Count > 0)
            {
                ...
                if (!n.isLeaf())
                {
                    cDebug.WriteLine("searching Node<T> " + n.nodeId + ", from index " + startIndex);
                    bool contains = false;
        ...
        private Node<T> splitNode(Node<T> n, Rectangle newRect, int newId)
        {
            ...
            if (INTERNAL_CONSISTENCY_CHECKING)
            {
                ...
                {
                    cDebug.WriteLine("Error: splitNode old Node<T> MBR wrong");
                }

                if (!newNode.mbr.Equals(calculateMBR(newNode)))
                {
                    cDebug.WriteLine("Error: splitNode new Node<T> MBR wrong");
                }
            }
        ...
        private void pickSeeds(Node<T> n, Rectangle newRect, int newId, Node<T> newNode)
        {
            ...
            for (int d = 0; d < Rectangle.DIMENSIONS; d++)
            {
                ...
                for (int i = 0; i < n.entryCount; i++)
                {
                    ...
                    {
                        cDebug.WriteLine("Invalid normalized separation");
                    }
        ...
        private int pickNext(Node<T> n, Node<T> newNode)
        {
            ...
            for (int i = 0; i < maxNodeEntries; i++)
            {
                if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED)
                {
                    ...
                    if (n.entries[i] == null)
                    {
                        cDebug.WriteLine("Error: Node<T> " + n.nodeId + ", entry " + i + " is null");
                    }
        ...
        private Node<T> chooseNode(Rectangle r, int level)
        {
            ...
            while (true)
            {
                if (n == null)
                {
                    cDebug.WriteLine("Could not get root Node<T> (" + rootNodeId + ")");
                }
        ...
        private Node<T> adjustTree(Node<T> n, Node<T> nn)
        {
            ...
            while (n.level != treeHeight)
            {
                ...
                {
                    cDebug.WriteLine("Error: entry " + entry + " in Node<T> " +
        ...
        private void checkConsistency(int nodeId, int expectedLevel, Rectangle expectedMBR)
        {
            ...
            if (n == null)
            {
                cDebug.WriteLine("Error: Could not read Node<T> " + nodeId);
            }

            if (n.level != expectedLevel)
            {
                cDebug.WriteLine("Error: Node<T> " + nodeId + ", expected level " + expectedLevel + ", actual level " + n.level);
            }
            ...
            {
                cDebug.WriteLine("Error: Node<T> " + nodeId + ", calculated MBR does not equal stored MBR");
            }

            if (expectedMBR != null && !n.mbr.Equals(expectedMBR))
            {
                cDebug.WriteLine("Error: Node<T> " + nodeId + ", expected MBR (from parent) does not equal stored MBR");
            }
            ...
            {
                cDebug.WriteLine("Error: Node<T> " + nodeId + " MBR using same rectangle object as parent's entry");
            }
            ...
            for (int i = 0; i < n.entryCount; i++)
            {
                if (n.entries[i] == null)
                {
                    cDebug.WriteLine("Error: Node<T> " + nodeId + ", Entry " + i + " is null");
                }
