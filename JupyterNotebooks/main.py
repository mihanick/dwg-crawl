...
from model import Trainer, DecoderRNN, EncoderRNN
...
def run(batch_size=32, pickle_file='test_dataset_cluster_labeled.pickle', lr=0.008, epochs=5, train_verbose=True, limit_seq_len=200):
    ...
    test_kl, test_rl = CalculateLoaderAccuracy(trainer.encoder, trainer.decoder, trainer.test_loader, trainer.device)
    print('Test losses rl:{:1.4f} kl:{:1.4f}'.format(test_rl, test_kl))

    return train_rl_losses, train_kl_losses, val_rl_losses, val_kl_losses, test_rl, test_kl
